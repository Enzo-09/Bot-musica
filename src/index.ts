...
import { Poru, NodeGroup } from "poru";
...
client.on("interactionCreate", async (interaction) => {
  ...
  try { await handle(interaction); } catch (e) { console.error(e); if (!interaction.replied) await interaction.reply({ content: "Hubo un error.", ephemeral: true }); }
});
...
async function handle(i: ChatInputCommandInteraction) {
  ...
  switch (i.commandName) {
    case "join": {
      ...
      if (!vc) return i.reply({ content: "Entrá a un canal de voz.", ephemeral: true });
...
case "play": {
  ...
  if (res.loadType === "playlist") {
    for (const t of res.tracks) {
      t.info.requester = i.user as any;
      player.queue.add(t);
    }
  ...
  return;
    ...
    case "skip": {
      ...
      if (!p) return i.reply({ content: "Nada sonando.", ephemeral: true });
      await p.skip();
    ...
    case "stop": {
      const p = poru.players.get(i.guildId!);
      if (!p) return i.reply({ content: "Nada que detener.", ephemeral: true });
      p.queue.clear();
      if (p.isPlaying) await p.skip();
      return i.reply("⏹ Stop");
    }
    ...
    case "np": {
      ...
      if (!c) return i.reply({ content: "Nada sonando.", ephemeral: true });
